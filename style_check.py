...
ERROR_REGEXPS = {
    ...
    "indent_with_spaces": re.compile(r"^( +)\S"),
    "tab_followed_by_space": re.compile(r"\t "),
...
def get_files(args):
    ...
    if args['<file>']:
        ...
        # Fallback to scanning the current folder for .c files
        c_files = []
        for file in os.listdir(os.getcwd()):  # Use os.getcwd() to get current folder
            if file.endswith(".c"):
                c_files.append(file)
        return c_files
